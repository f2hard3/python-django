...
class TestPostViewSet:
    ...
    def test_retrieve(self, client, user, post):
        client.force_authenticate(user=user)
        response = client.get(f'{self.endpoint}{str(post.public_id)}/')
    ...
    def test_update(self, client, user, post):
        ...
        response = client.put(f'{self.endpoint}{str(post.public_id)}/', data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data['body'] == data['body']

    def test_delete(self, client, user, post):
        client.force_authenticate(user=user)
        response = client.delete(f'{self.endpoint}{str(post.public_id)}/')
