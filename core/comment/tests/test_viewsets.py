...
class TestCommentViewSet:
    ...
    def test_list(self, client, user, post, comment):
        ...
        response = client.get(f'{self.endpoint}{str(post.public_id)}/comment/')
        assert response.status_code == status.HTTP_200_OK
        assert response.data['count'] == 1

    def test_retrieve(self, client, user, post, comment):
        client.force_authenticate(user=user)
        response = client.get(f'{self.endpoint}{str(post.public_id)}/comment/{str(comment.public_id)}/')
    ...
    def test_create(self, client, user, post):
        ...
        response = client.post(f'{self.endpoint}{str(post.public_id)}/comment/', data)
    ...
    def test_update(self, client, user, post, comment):
        ...
        response = client.put(f'{self.endpoint}{str(post.public_id)}/comment/{comment.public_id}/', data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data['body'] == data['body']

    def test_delete(self, client, user, post, comment):
        client.force_authenticate(user=user)
        response = client.delete(f'{self.endpoint}{str(post.public_id)}/comment/{comment.public_id}/')
